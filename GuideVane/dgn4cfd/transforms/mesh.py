...
def edge_pruning(
...
) -> Tuple[torch.LongTensor, Union[torch.Tensor, None]]:
    ...
    Returns:
        Tuple[torch.LongTensor, Union[torch.Tensor, None]]: The pruned edge index and edge attribute tensors (if provided).
...
class MeshCoarsening:
    ...
    Args:
        ...
        max_indegree (int, optional): Maximum in-degree allowed. Defaults to None.
        rel_pos_scaling (List[float], optional): Scaling factor for the relative position in the lower-resolution graph. Defaults to None.
        scalar_rel_pos (bool, optional): Whether to use the scalar relative position (distance) or the vector relative position. Defaults to False (vector relative position).
    """
    ...
    def __init__(
        ...
        num_scales:      int,
        max_indegree:    int               = None,
        rel_pos_scaling: Optional[List[float]] = None,
        scalar_rel_pos:  bool              = False,
    ) -> None:
